define(['app', 'kendo','sync'], function (app,kendo) {
    "use strict";
 app.factory('commonDataSource', ['$http', '$q', 'sync', 'url', function ($http, $q, $sync, url) {
        
        var omitKeys = ["filter", "skip", "sort", "take"];
       
        var commonOnlineTransport = function (options) {
        ...
        commonOnlineTransport.prototype.read = function (options) {
            var transport = this,
        ...
        commonOnlineTransport.prototype.create = function (options) {
            var transport = this,
                parentDs = this.parentDs;
            if (this.otherData) {
        ...
        commonOnlineTransport.prototype.update = function (options) {
            var transport = this,
                parentDs = this.parentDs;
            var idPro = "id";
            if(transport.idPro){
        ...
        commonOnlineTransport.prototype.destroy = function (options) {
          // 逻辑上根据后台返回结果决定是否删除，因此把改方法的请求移动到preDestroy中
          options.success();
        };
        commonOnlineTransport.prototype.preDestroy = function (data) {
            var transport = this,
            parentDs = this.parentDs;
        ...
        var commonDataSource = kendo.data.DataSource.extend({
            init: function (options) {
                var trans = {},
                    customerOptions = {autoSync: false};
                customerOptions.transport = new commonOnlineTransport(options);
        ...
        return function DsFactory(options) {
            ...
            ds = new commonDataSource(options);
